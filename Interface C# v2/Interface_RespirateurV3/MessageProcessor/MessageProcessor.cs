...
namespace MessageProcessor
{
    public class MsgProcessor
    {
        ...
        public void ProcessDecodedMessage(Int16 command, Int16 payloadLength, byte[] payload)
        {
            ...
            switch (command)
            {
                ...
                case (short)Commands.SetSeuilAssistance:
                    ...
                    break;
                case (short)Commands.SetUnsupervisedLearningParameters:
                    int ClustersNumber = payload[0];
                    int AnomaliesNumber = payload[1];
                    int StateTraining = payload[2];
                    OnSetUnsupervisedLearningParameters(ClustersNumber, AnomaliesNumber, StateTraining);
                    break;
                case (short)Commands.SetClusterInfo:
                    int TotalClustersNumber = payload[0];
                    int ClusterDimension = payload[1];
                    double[,] Clusters = new double[TotalClustersNumber, ClusterDimension];
                    double[,] Dispersions = new double[TotalClustersNumber, ClusterDimension];
                    double[] NumberOfSamplesInCluster = new double[TotalClustersNumber];
                    int ClusterIndex = payload[2];
                    for (int i = 0; i < ClusterDimension; i++)
                    {
                        //Clusters[ClusterIndex, i] = (double)((((UInt16)payload[3 + i * 3]) << 8) + (UInt16)payload[4 + i * 3]) + ((double)payload[5 + i * 3]) / 100;
                        Clusters[ClusterIndex, i] = (double)((((UInt16)payload[3 + i * 8]) << 8) + (UInt16)payload[4 + i * 8]) + ((double)payload[5 + i * 8]) / 100;
                        Dispersions[ClusterIndex, i] = (double)((((UInt16)payload[6 + i * 8]) << 8) + (UInt16)payload[7 + i * 8]) + ((double)payload[8 + i * 8]) / 100;
                        NumberOfSamplesInCluster[ClusterIndex] = (double)((((UInt16)payload[9 + i * 8]) << 8) + (UInt16)payload[10 + i * 8]);
                    }


                    OnSetUnsupervisedLearningClusterInfo(TotalClustersNumber, ClusterDimension, Clusters, Dispersions, NumberOfSamplesInCluster, ClusterIndex);

                    break;
                case (short)Commands.SetSampleInfo:
                    int SampleDimension = payload[0];
                    double[] Sample = new double[SampleDimension];
                    for (int i = 0; i < SampleDimension; i++)
                        Sample[i] = (double)((((UInt16)payload[1 + i * 3]) << 8) + (UInt16)payload[2 + i * 3]) + ((double)payload[3 + i * 3]) / 100;

                    OnSetUnsupervisedLearningSampleInfo(SampleDimension, Sample);

                    break;
                case (short)Commands.AlarmSystem:
        ...
        public event EventHandler<UnsupervisedLearningParametersArgs> OnSetUnsupervisedLearningParametersEvent;
        public virtual void OnSetUnsupervisedLearningParameters(int clustersNumber, int anomaliesNumber, int stateTraining)
        {
            var handler = OnSetUnsupervisedLearningParametersEvent;
            if (handler != null)
            {
                handler(this, new UnsupervisedLearningParametersArgs { numberOfAnomalies = anomaliesNumber,
                    numberOfClusters = clustersNumber, trainingState = stateTraining});
            }
        }
        public event EventHandler<UnsupervisedLearningParametersArgs> OnSetUnsupervisedLearningClusterInfoEvent;
        //public virtual void OnSetUnsupervisedLearningClusterInfo(int clustersNumber, int clustersDimension, double[,] clusters, int clusterIndex)
        //{
        //    var handler = OnSetUnsupervisedLearningClusterInfoEvent;
        //    if (handler != null)
        //    {
        //        handler(this, new UnsupervisedLearningParametersArgs
        //        {
        //            numberOfClusters = clustersNumber,
        //            ClusterIndex = clusterIndex,
        //            Clusters = clusters,
        //            ClustersDimension = clustersDimension
        //        });
        //    }
        //}
        //TODO supprimer une des deux fonctionnes 
        public virtual void OnSetUnsupervisedLearningClusterInfo(int clustersNumber, int clustersDimension, double[,] clusters, double[,] dispersions, double[] numberOfSamplesOnCluster, int clusterIndex)
        {
            var handler = OnSetUnsupervisedLearningClusterInfoEvent;
            if (handler != null)
            {
                handler(this, new UnsupervisedLearningParametersArgs
                {
                    ...
                    NumberOfSamplesInCluster = numberOfSamplesOnCluster,
                    ClustersDimension = clustersDimension
                });
            }
        }
        public event EventHandler<UnsupervisedLearningParametersArgs> OnSetUnsupervisedLearningSampleInfoEvent;
        public virtual void OnSetUnsupervisedLearningSampleInfo(int sampleDimension, double[] sample)
        {
            var handler = OnSetUnsupervisedLearningSampleInfoEvent;
            if (handler != null)
            {
                handler(this, new UnsupervisedLearningParametersArgs
                {
        ...
        public event EventHandler<UnsupervisedLearningParametersArgs> OnSetAlarmSystemEvent;
        public virtual void OnSetAlarmSystem()
        {
            var handler = OnSetAlarmSystemEvent;
            if (handler != null)
            {
                handler(this, new UnsupervisedLearningParametersArgs
                {
