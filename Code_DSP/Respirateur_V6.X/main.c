...
extern float Clusters[MaxQClustersC][NeuronsInputLayerC];
extern float Dispersions[MaxQClustersC][NeuronsInputLayerC];
extern float SamplesCounter[MaxQClustersC];
...
void ResetLearning(void);
void newSampleUnsupervisedLearning(void);
void SendClustersInfo(void);
BOOL DetectSuccessiveAnomalies(int anomaliesResult);
void ResetLearningAfterParametersModification(void);
...
//Features extraction and unsupervised learning variables 
BOOL isTheFirstSample = true;
BOOL DoingFeaturesExtracion = false;
BOOL TrainingFinished = false;
float VolumePeak = 0;
float VolumeArea = 0;
float VolumePulseDuration = 0;
float PressurePeak = 0;
float PressureArea = 0;
float PressurePulseDuration = 0;
...
int main(void)
{
    ...
    /****************************************************************************************************/
    // Initialization Unsupervised Learning
    /****************************************************************************************************/
    //InitUnsupervisedKMeansModif(1, 0.1, 1);
...
void StateMachineRespirateur3Moteurs(void)
{
    switch(etat)
        ...
        case INIT_0:
            ...
            case SOUFFLE:
                if(!isTheFirstSample){
                    //Stop Features extraction for Unsupervised Learning 
                    DoingFeaturesExtracion = false;                     
                    newSampleUnsupervisedLearning();
                    //Start Features extraction for Unsupervised Learning 
                    DoingFeaturesExtracion = true;                                                    
                }
...
void Timer5CallBack(void)
{
    ...
    //Features extraction for Unsupervised Learning from pressure and volume 
    if(DoingFeaturesExtracion){
        //Volume
        //Pulse duration, the timer is @ 50 HZ
        VolumePulseDuration += 0.02;
        //Peak        
        if((float)respiratorState.volumeCourant>VolumePeak)
          VolumePeak = (float)respiratorState.volumeCourant;
        //Area
        VolumeArea += ((float)respiratorState.volumeCourant)*0.02;
        //Pressure
        //Pulse duration, the timer is @ 50 HZ
        PressurePulseDuration += 0.02;
        //Peak        
        if((float)respiratorState.pressure2>PressurePeak)
          PressurePeak = (float)respiratorState.pressure2;
        //Area
        PressureArea += ((float)respiratorState.pressure2)*0.02;
    } 
...
//<editor-fold defaultstate="collapsed" desc="Cluster Input Variables">

    float UnsupervisedLearningSample[6];    
//    UnsupervisedLearningSample[0] = VolumePeak;
//    UnsupervisedLearningSample[1] = VolumeArea;
//    UnsupervisedLearningSample[2] = VolumePulseDuration;
//    UnsupervisedLearningSample[3] = PressurePeak;
//    UnsupervisedLearningSample[4] = PressureArea;
//    UnsupervisedLearningSample[5] = PressurePulseDuration;

// Clusters reduced to three dimensions     
    if(VolumeArea>0)
        UnsupervisedLearningSample[0] = VolumeArea;
    else
        UnsupervisedLearningSample[0] = 0.01f;

    if(PressurePeak>0)
        UnsupervisedLearningSample[1] = PressurePeak;
    else
        UnsupervisedLearningSample[1] = 0.01f;
    
    if(PressureArea>0)
        UnsupervisedLearningSample[2] = PressureArea;
    else
        UnsupervisedLearningSample[2] = 0.01f;
//</editor-fold>

    
     if(!TrainingFinished){//Algorithm is training 
         int TrainingResult = Training(UnsupervisedLearningSample);
         ...
         //Preparer la commande et envoyer les données du training en cours
         unsigned char payload[16];
         payload[0] = NumberOfClusters; //Number of clusters
         payload[1] = 0;                //Number of anomalies
         payload[2] = 1;                //1 = Training, 0 = Detection
         MakeAndSendMessageWithUTLNProtocol(0x0028, 16, payload);
         
        //Send Sample info
         int sampleDimension = GetClusterDimension();
         unsigned char payloadSample[1 + sampleDimension*3];
         payloadSample[0] = sampleDimension; 
         int dimensionIndex = 0;
         for(dimensionIndex = 0; dimensionIndex < sampleDimension; dimensionIndex++){
             payloadSample[1 + dimensionIndex*3] = (uint8) ((uint16)UnsupervisedLearningSample[dimensionIndex]>>8);
             payloadSample[2 + dimensionIndex*3] = (uint8) ((uint16)UnsupervisedLearningSample[dimensionIndex]);
             payloadSample[3 + dimensionIndex*3] = (uint8) ((UnsupervisedLearningSample[dimensionIndex] - (uint16)UnsupervisedLearningSample[dimensionIndex])*100);
         }
         MakeAndSendMessageWithUTLNProtocol(0x002A, 1 + sampleDimension*3, payloadSample);
        SendClustersInfo();
         ...
         int AnomaliesResult = AnomalyDetection(UnsupervisedLearningSample);
         BOOL alert = DetectSuccessiveAnomalies(AnomaliesResult);
         if(!alert){
            SendClustersInfo();
            int NumberOfClusters = GetNumberOfClusters();
            //Prepare la commande et envoyer
            unsigned char payload[16];
            payload[0] = NumberOfClusters;
            payload[1] = AnomaliesResult;
            payload[2] = 0;
            MakeAndSendMessageWithUTLNProtocol(0x0028, 16, payload);
         }
     }
     ...
     VolumePeak = 0;
     VolumeArea = 0;
     VolumePulseDuration = 0;
     PressurePeak = 0;
     PressureArea = 0;
     PressurePulseDuration = 0;
}
...
void SendClustersInfo(void){
    ...
    int NumberOfClusters = GetNumberOfClusters();
    int clusterIndex = 0;
    for(clusterIndex = 0; clusterIndex<NumberOfClusters; clusterIndex++){
       //Preparer la commande et envoyer
       int clusterDimension = GetClusterDimension();
       unsigned char payload[3 + clusterDimension*8];
       payload[0] = NumberOfClusters; //Total number of clusters
       payload[1] = clusterDimension;
       payload[2] = clusterIndex;
       int dimensionIndex = 0;
       for(dimensionIndex = 0; dimensionIndex < clusterDimension; dimensionIndex++){
           //Clusters
           payload[3 + dimensionIndex*8] = (uint8) ((uint16)Clusters[clusterIndex][dimensionIndex]>>8);
           payload[4 + dimensionIndex*8] = (uint8) ((uint16)Clusters[clusterIndex][dimensionIndex]);
           payload[5 + dimensionIndex*8] = (uint8) ((Clusters[clusterIndex][dimensionIndex] - (uint16)Clusters[clusterIndex][dimensionIndex])*100);
           //Dispersions
           payload[6 + dimensionIndex*8] = (uint8) ((uint16)Dispersions[clusterIndex][dimensionIndex]>>8);
           payload[7 + dimensionIndex*8] = (uint8) ((uint16)Dispersions[clusterIndex][dimensionIndex]);
           payload[8 + dimensionIndex*8] = (uint8) ((Dispersions[clusterIndex][dimensionIndex] - (uint16)Dispersions[clusterIndex][dimensionIndex])*100);
           //Samples on each Cluster
           payload[9 + dimensionIndex*8] = (uint8) ((uint16)SamplesCounter[clusterIndex]>>8);
           payload[10 + dimensionIndex*8] = (uint8) ((uint16)SamplesCounter[clusterIndex]);
       }
       MakeAndSendMessageWithUTLNProtocol(0x0029, 10 + clusterDimension*8, payload);   
    }
}
...
BOOL DetectSuccessiveAnomalies(int anomaliesResult){
    ...
    BOOL Alert = false;    
        if(NumberofAnomalies == 0){
             //If there were not anomalies we stay here waiting for one
             NumberofAnomalies = anomaliesResult;
        }
        else{
             if(anomaliesResult - NumberofAnomalies > 0){
                //If a new anomaly was detected we increase the number of successive anomalies
                successiveAnomalies+=2;
                NumberofAnomalies = anomaliesResult;
             }
             else{
                //If not new anomalies were detected we decrease the counter of successive anomalies
        ...
        if(successiveAnomalies > 4){
            //Reinit the system
            //InitMachine();
            //Stop the system
            //startRespirator=false;
            unsigned char payload2[1];
            payload2[0] = startRespirator;
            MakeAndSendMessageWithUTLNProtocol(0x0001, 1, payload2);
            //Send Alarm 
            unsigned char payload[16];
            payload[0] = successiveAnomalies; //Number of anomalies
            MakeAndSendMessageWithUTLNProtocol(0x002B, 16, payload);
            ResetLearning();
            int i;
            for(i=0;i<MaxQClustersC;i++)
                SamplesCounter[i] = 0.0f;
             isTheFirstSample = true;
             DoingFeaturesExtracion = false;
